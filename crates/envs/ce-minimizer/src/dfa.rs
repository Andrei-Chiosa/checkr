...
impl NamedDFA {
    ...
    pub fn to_dot(&self) -> String {
        ...
        for (node, state) in enumerate(&self.names) {
            s.push_str(&format!("  {} [label=\"{}\"", node, state));
            if self.dfa.initial == node {
                s.push_str(", isInitial=true")
            };
            if self.dfa.accepting.contains(&node) {
                s.push_str(", isAccepting=true")
            };
            s.push_str("];\n");
        }
        s.push_str("\n");
    ...
    pub fn from_dot(dot: &String) -> Result<Self, Error> {
        ...
        for line in dot.lines() {
            ...
            if line.contains("->") {
                ...
                let from: Node = parts[0].trim().parse().map_err(|_| Error::InvalidInput {
                ...
                let symbols = line.split('"').nth(1).ok_or(Error::InvalidInput {
                    message: "invalid transition".to_string(),
                })?;
                for sym in symbols.split(',') {
                    let c = sym.trim().chars().next().ok_or(Error::InvalidInput {
                        message: "invalid transition".to_string(),
                    })?;
                    alphabet.insert(c);
                    edges.push(Edge {
                ...
                if line.contains("isInitial=true") {
                    initial = node;
                }
                if line.contains("isAccepting=true") {
                    accepting.push(node);
                }
            }
        }
